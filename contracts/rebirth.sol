...
// Contract devs:
//
// @seanking52, @TrevorLaheyofficiaL, @JigsawOfficial
//
...
contract Rebirth is ERC20, Ownable {
    ...
    uint256 public maxPrivSaleSell = .50 ether;
    ...
    constructor() payable ERC20("Rebirth", "REBIRTH") {
        ...
        maxBuyAmount = (totalSupply * 6) / 10000; // 0.06%
        maxSellAmount = (totalSupply * 6) / 10000; // 0.06%
        ...
        operationsAddress = address(0xBeaf78AeF141B8A4c5e7649236a4d4a29A027D47);
        treasuryAddress = address(0x043cBAE86Aba5c5f2ca0B71F0B10695a7CcbAffF);
        ...
        _excludeFromMaxTransaction(
            0xD09E0d18d07eedF8c12A237EC4c70d0f810f03d2,
            true
        ); // Exchanges wallet
        ...
        excludeFromFees(0xD09E0d18d07eedF8c12A237EC4c70d0f810f03d2, true);

        _createInitialSupply(newOwner, (totalSupply * 42) / 1000); // Tokens for private salers
        _createInitialSupply(
            0xD09E0d18d07eedF8c12A237EC4c70d0f810f03d2,
            (totalSupply * 5) / 100
        ); // Exchanges wallet
        _createInitialSupply(address(0xdead), (totalSupply * 878) / 1000); // Burn
    ...
    function letsFuckingGo(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            ...
            nextPrivateWalletSellDate[wallet] = block.timestamp + 3 hours; // No sales in the first 6 hours after launch
            uint256 amount = amountsInTokens[i];
            super._transfer(msg.sender, wallet, amount);
        }
